...
Elm.Main.make = function (_elm) {
   ...
   var isEven = function (num) {    return _U.eq(A2($Basics._op["%"],num,2),0);};
   var funnyFunction = function (_p0) {    return $Basics.not(isEven(A2(F2(function (x,y) {    return x + y;}),1,_p0)));};
   var view = F2(function (address,string) {
      return A2($Html.div,_U.list([]),_U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(funnyFunction(3) ? "1" : "0")]))]));
   });
   var main = $StartApp$Simple.start({model: "",view: view,update: update});
   return _elm.Main.values = {_op: _op,main: main,isEven: isEven,funnyFunction: funnyFunction,view: view,update: update};
};
